...
\title{AI-Driven Integration Testing for Game Servers via Deterministic State Machine Breakpoints}
...
\item In 42 runs on an event-driven system prototype, GLM-5.1's step-wise group achieves F1=0.74 (vs.\ batch group 0.32), GPT-5.4's code+batch group performs best (5.0/7), and L1+LSP on GPT-5.4 reaches a single-run maximum of 6/7 (Section~5).
\end{enumerate}
...
Level Score & L1=1.0, L2=1.5, L3=2.0, L4=3.0; max 14.0 \\
Correlation Recall & Correct cross-module correlations / 10 \\
False Positive Rate & False positives / total reported defects \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Ablation factor analysis (based on 3-run means):}

\begin{table}[t]
\caption{Ablation factor analysis (GLM-5.1).}
\label{tab:glm-ablation}
\small
\begin{tabular}{lc}
\toprule
Factor & Bug Recall Gain \\
\midrule
Step-wise main effect (B$-$A vs D$-$C) & +0.24 \\
...
\textbf{Finding 5: B4 and B6 are the most easily detected defects on both models.} On GLM-5.1, B4 (sign-in reward without idempotency guard) was detected in all 15 L2 runs; B6 (mail attachment chain break) was detected in 12/15 runs. The behavioral anomalies of these two defects are highly visible in logs and require no deep reasoning to identify.
...
\textbf{Core insight:} Step-wise execution and code understanding each make independent contributions, but their relative magnitudes depend on model capability. On GLM-5.1, the marginal gain from step-wise execution (+0.24 recall) exceeds that of code understanding (+0.18), while on GPT-5.4, code understanding dramatically improves batch mode performance (C group 5.0 vs A group 1.7). Both models exhibit negative interaction effects, indicating that simultaneously enabling both capabilities creates cognitive load issues under the current agent architecture. This implies that as LLM capabilities improve, the optimal strategy should adjust the weight allocation between code understanding and runtime exploration based on model characteristics.
...
\textbf{Metric appropriateness:} Whether F1 score and level-weighted score adequately reflect test quality merits discussion. For example, difficulty weight assignments (L1=1.0, L2=1.5, L3=2.0, L4=3.0) are based on author judgment and may be subjective.

\textbf{Defect judgment criteria:} Some agent-reported findings require human judgment on whether they constitute true defects. Consistency of judgment criteria may affect the accuracy of precision and false positive rates.
...
This paper presents BST-Agent, an agent-driven integration testing method based on tree-structured command space and step-wise reasoning. Through experimental validation on two LLMs (GLM-5.1, GPT-5.4), we draw the following conclusions:
...
\item The effectiveness of code understanding depends heavily on model capability---on GLM-5.1, the marginal gain of code understanding (+0.18) is less than step-wise execution (+0.24), while on GPT-5.4, code understanding lifts batch mode from 1.7 to 5.0.
