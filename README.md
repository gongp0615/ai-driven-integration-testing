# BST-Agent：基于命令树与步进思考的 Agent 驱动集成测试方法
...
大语言模型（LLM）大幅提升了代码生成效率，但系统级验证仍高度依赖人工编写测试用例。随着开发速度加快，模块间关联日益复杂，传统方法面临结构性瓶颈。本文提出 BST-Agent，一种面向复杂系统的 Agent 驱动集成测试方法，通过三阶段流程（理解—设计—执行）将大部分系统级验收工作从开发者转移到 Agent。该方法基于两个核心机制：（1）树状命令空间——将被测系统暴露为"模块→命令→参数表"的层级结构，Agent 既能使用已有命令，也能在受控边界内自主注册新命令以扩展测试能力；（2）步进思考——通过断点控制器将执行拆分为单步，使 Agent 在每步之后观察因果链并动态调整测试方向。在一个包含 6 个业务模块和 7 个预埋缺陷的事件驱动游戏服务端原型上，我们设计了三级自主性模式（L0/L1/L2）和 2×2 消融实验，在两个 LLM（GLM-5.1、GPT-5.4）上各进行 3 轮重复实验（共 42 次运行）。实验结果表明：GLM-5.1 上步进执行组平均发现 3.7/7 个缺陷（F1=0.74），批量执行组仅 1.3/7（F1=0.32）；GPT-5.4 上代码+批量组表现最强（5.0/7）；L0 模式下两个模型均能从零注册完整测试命令集并发现 4-5 个缺陷，L1 模式在 GPT-5.4 上单次最高发现 6/7 个缺陷，验证了方法的跨模型可行性。
...
4. 在事件驱动系统原型上的 42 次运行中，GLM-5.1 步进组 F1 达 0.74（vs. 批量组 0.32），GPT-5.4 代码+批量组最优（5.0/7），L1+LSP 在 GPT-5.4 上单次最高达 6/7（第 5 节）。
...
| Achievement | 成就解锁 | 订阅 `task.completed`、`item.added`、`equip.success` |
...
| F1 值 | 精确率与召回率的调和平均 |
| 难度加权分（Level Score） | L1 权重 1.0，L2 权重 1.5，L3 权重 2.0，L4 权重 3.0，满分 14.0 |
| 关联发现率（Correlation Recall） | 正确识别的跨模块关联数 / 10 |
| 误报率（FP Rate） | 误报缺陷数 / 报告的总缺陷数 |
...
| 因子 | 缺陷召回率增益 |
|------|:-------------:|
| 步进执行主效应（B−A vs D−C） | +0.24 |
| 代码理解主效应（C−A vs D−B） | +0.18 |
| 交互效应 | −0.24 |
...
**发现 5：B4 和 B6 是两个模型上最容易被发现的缺陷。** GLM-5.1 上 B4（签到奖励无幂等保护）在全部 15 次 L2 运行中均被发现，B6（邮件附件断链）在 12/15 次运行中被发现。这两个缺陷的行为异常在日志中高度可见，无需深层推理即可识别。
...
**核心洞察：** 步进执行和代码理解各有独立贡献，但效果的相对大小取决于模型能力。GLM-5.1 上步进执行的边际收益（+0.24 召回率）大于代码理解（+0.18），而 GPT-5.4 上代码理解显著提升了 batch 模式的表现（C 组 5.0 vs A 组 1.7）。两个模型上均观察到负交互效应，说明同时启用两种能力在当前 Agent 架构下存在认知负荷问题。这意味着随着 LLM 能力提升，最优策略需要根据模型特点调整代码理解与运行时探索的权重分配。
...
GLM-5.1 上 D 组（2.7/7）表现低于 B 组和 C 组（均 3.7/7），GPT-5.4 上 C 组（code-batch，5.0/7）显著优于 B 组（step-only，2.0/7）。这一跨模型对比揭示了一个重要洞察：代码理解的边际收益不是固定的，而是随模型的代码理解能力增长。两个模型上均观察到的负交互效应部分源于同时启用代码分析和步进执行增加了 Agent 的认知负荷——在代码阅读与运行时探索间的切换消耗了额外迭代。当模型代码理解能力更强（GPT-5.4）或工具更高效（LSP 语义导航替代文本搜索）时，代码理解阶段的投入回报率会显著提升。
...
**评估指标的适当性：** F1 值和难度加权分是否能充分反映测试质量值得讨论。例如，难度权重的设定（L1=1.0, L2=1.5, L3=2.0, L4=3.0）基于作者判断，可能存在主观性。

**缺陷判定标准：** Agent 报告中有些发现需要人工判定是否构成真实缺陷。判定标准的一致性可能影响精确率和误报率的准确性。
...
本文提出 BST-Agent，一种基于树状命令空间和步进思考的 Agent 驱动集成测试方法。通过在两个 LLM（GLM-5.1、GPT-5.4）上的实验验证，得出以下结论：

1. 步进执行和代码理解均有独立贡献——GLM-5.1 上步进组和代码+批量组均达 3.7/7（vs. 批量组 1.3/7），GPT-5.4 上代码+批量组最优（5.0/7）；
2. 代码理解的效果高度依赖模型能力——GLM-5.1 上代码理解的边际收益（+0.18）小于步进执行（+0.24），而 GPT-5.4 上代码理解使 batch 模式从 1.7 提升到 5.0；
...
该日志完整展示了 BST-Agent 的三个核心特征：（1）`[Thinking]` 揭示了世界暂停期间的推理链——Agent 在每步之后分析因果、提出假设、决定下一步验证方向；（2）Agent 在暂停窗口中主动查询关联模块追踪副作用，这些决策完全是运行时动态生成的；（3）Agent 通过"操作→观察→思考→再操作"的循环逐步收敛到缺陷，而非预先规划测试路径。
