...
4. Across 42 runs on an event-driven system prototype, the GLM-5.1 step-wise group achieved F1=0.74 (vs. batch group 0.32), GPT-5.4 code+batch group was optimal (5.0/7), and L1+LSP on GPT-5.4 achieved a single-run maximum of 6/7 (Section 5).
...
The log above demonstrates three key characteristics of the BST-Agent workflow:
...
**Appropriateness of evaluation metrics:** Whether F1 scores and level-weighted scores adequately reflect testing quality merits discussion. For example, the difficulty weight assignments (L1=1.0, L2=1.5, L3=2.0, L4=3.0) are based on author judgment and may be subjective.

**Defect adjudication criteria:** Some findings in agent reports required human judgment on whether they constitute real defects. The consistency of adjudication criteria may affect the accuracy of precision and false positive rates.
...
This paper proposes BST-Agent, an agent-driven integration testing method based on a tree-structured command space and step-wise reasoning. Through experimental validation on two LLMs (GLM-5.1, GPT-5.4), the following conclusions are drawn:
